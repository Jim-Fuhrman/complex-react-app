          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : '')}>
            {Boolean(state.results.length) && (
              ...
              {state.results.map(post => {
                ...
                return (
                  <Link onClick={() => appDispatch({type: "closeSearch"})} key={post._id} to={`/post/${post._id}`} class="list-group-item list-group-item-action">
                    <img class="avatar-tiny" src={post.author.avatar} /> <strong>{post.title}</strong> <span class="text-muted small">by {post.author.username} on {dateFormatted} </span>
                  </Link>
                )
              })}
            </div>
            )}
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm">Sorry, no search results found</p>}
          </div>
        </div>
      </div>
    </div>
